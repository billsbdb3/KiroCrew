...
class TestTheFenceActuallyCoversThisPath:
    ...
    def test_the_dotenv_path_is_fenced_from_the_agent(self, monkeypatch, tmp_path):
        ...
        # BOTH, because `expanduser("~")` reads `USERPROFILE` on Windows and `HOME`
        # on POSIX. Setting only `HOME` left the fence anchored on the runner's real
        # home, so the path under test was not beneath it and the assertion below
        # reported the fence as missing -- green on macOS, red on Windows, and the
        # failure message accused `_SENSITIVE_HOME_DIRS` of drifting when the test
        # was simply looking at the wrong home.
