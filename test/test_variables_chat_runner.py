...
def _expand(message: str, values: dict[str, str], slot: _ChatSlot, state: MagicMock) -> str:
    ...
    with patch.object(chat_runner, "resolve_variables") as rv:
        ...
        with patch.object(chat_runner.KiroCrewConfig, "load", classmethod(lambda cls: MagicMock())):
            return chat_runner._expand_message_variables(message, state, slot)[0]


def _expand_full(
    message: str, values: dict[str, str], slot: _ChatSlot, state: MagicMock
) -> tuple[str, list[str]]:
    """Like :func:`_expand`, but hands back the unresolved names as well.

    Same wiring as `_expand`; only the return differs, so the two cannot disagree about
    how the expander is invoked.
    """
    with patch.object(chat_runner, "resolve_variables") as rv:
        rv.return_value = MagicMock(values=values)
        with patch.object(chat_runner.KiroCrewConfig, "load", classmethod(lambda cls: MagicMock())):
...
class TestExpanderBehaviour:
    def test_no_token_short_circuits_without_resolving(self):
        state, slot = _state(), _slot()
        with patch.object(chat_runner, "resolve_variables") as rv:
            out, _ = chat_runner._expand_message_variables("no tokens here", state, slot)
    ...
    def test_resolution_failure_leaves_the_message_intact(self):
        ...
        with patch.object(chat_runner, "resolve_variables", side_effect=RuntimeError("boom")):
            with patch.object(
                ...
                out, _ = chat_runner._expand_message_variables("use {{a}}", state, slot)
        assert out == "use {{a}}"

    def test_unresolved_name_is_reported_once_and_left_literal(self):
        """The expander RETURNS the unresolved names; it no longer appends them.

        `slot.append` ends in `asyncio.Event.set()`, and this function runs in a worker
        thread, where that call is not thread-safe. The surfacing still happens -- on
        the loop, at the call site, which `test_the_pipeline_surfaces_them_on_the_loop`
        pins.
        """
        state, slot = _state(), _slot()
        out, unresolved = _expand_full("use {{missing}}", {"other": "1"}, slot, state)
        assert out == "use {{missing}}", "the token must survive verbatim"
        assert unresolved == ["missing"]
        assert not [
            m for m in slot.messages if "missing" in str(m)
        ], "the worker thread must not touch slot state"

    def test_the_pipeline_surfaces_them_on_the_loop(self):
        """The other half: moving the append out of the worker must not lose it.

        Structural, because the append now lives inside `_run_chat` rather than in a
        function a unit test can call. Without this, deleting the caller's branch
        leaves every behavioural test above green and the operator never told that
        their variable was undefined.
        """
        region = inspect.getsource(runner_mod._run_chat)
        assert "_unresolved_vars" in region, "the pipeline discards the unresolved names"
        assert "Undefined variable(s) left as written" in region
        # Appended OUTSIDE the to_thread call, which is the whole point.
        offload = region.index("asyncio.to_thread(")
        append = region.index("Undefined variable(s) left as written")
        assert append > offload, "the surfacing must happen after the worker returns"
...
def test_pipeline_expands_before_joining_imported_bodies():
    ...
    # Not "_resolve_dollar_skills(" -- the stage is offloaded, so the name is an
    # argument to asyncio.to_thread and is not followed by a paren.
...
class TestTheMirrorPayloadIsCapturedNotReconstructed:
    ...
    def _sole_capture(self) -> int:
        ...
        hits = [
            ...
            if ln.strip() == "_user_msg_for_mirror = pre_expansion_message"
        ]
        assert len(hits) == 1, (
...
class TestRecoveryReplaysTheAuthoredText:
    """The invariant, after three rounds of approximating it.

    A turn that fails before streaming is re-queued, and the recovery path's own
    contract is to replay "the USER'S OWN message verbatim". It was replaying
    ``message``, which by then has been through `@prompt`, `$skill` and `{{name}}`
    expansion -- so a variable's VALUE reached the resolvers on the replay. Reported
    three times, once per trigger shape: a leading `@`, a `$skill` token anywhere, and
    a `/clear` slash command.

    The first two were answered by enumerating what a value must not become, which is
    the wrong shape of fix: `/` was simply the next character nobody had listed. The
    queue now carries ``pre_expansion_message`` -- captured before ANY resolution stage
    -- so the replay re-runs the whole pipeline from the operator's own text and a
    value can never be the thing that gets resolved. There is nothing left to
    enumerate.
    """

    def test_every_requeue_replays_the_authored_text(self):
        """Structural, and deliberately counts: a fifth requeue added later that reaches
        for `message` reopens this, and no behavioural test would notice."""
        region = inspect.getsource(runner_mod._run_chat)
        calls = region.count("build_recovery_requeue(")
        assert calls >= 4, f"expected the known requeue sites, found {calls}"
        authored = region.count("build_recovery_requeue(\n                pre_expansion_message,")
        authored += region.count(
            "build_recovery_requeue(\n                    pre_expansion_message,"
        )
        assert authored == calls, (
            f"{calls - authored} requeue site(s) replay the EXPANDED message; a "
            "variable's value would reach the prompt/skill/slash resolvers on retry"
        )

    def test_the_capture_precedes_every_resolution_stage(self):
        """`pre_expansion_message` is only the authored text while it is taken before
        the resolvers run. If a stage moves above it, the requeue silently starts
        carrying resolved text again."""
        region = inspect.getsource(runner_mod._run_chat)
        capture = region.index("pre_expansion_message = ")
        for needle in (
            'if message.startswith("@")',
            "_resolve_dollar_skills",
            "_expand_message_variables",
        ):
            assert region.index(needle) > capture, f"{needle} runs before the capture"

    def test_a_slash_command_value_cannot_reach_the_retry(self):
        """The third report's shape, as a property rather than a character check: the
        replayed text is the operator's, so whatever a value expands to is irrelevant."""
        region = inspect.getsource(runner_mod._run_chat)
