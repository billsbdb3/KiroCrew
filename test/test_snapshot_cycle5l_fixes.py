...
class TestATableThatCannotBeInspectedRefusesTheUpload:
    ...
    def test_no_branch_skips_a_table_it_could_not_read(self):
        ...
        assert src.count("_TableNotInspectable(") == 5, (
            "every state this pass cannot prove clean must refuse: a table with no rowid, "
            "a table with no columns, a database that never settles because a trigger keeps "
            "reintroducing values, and the two VALUES that cannot be rewritten without "
            "shifting bytes something else depends on -- a NUL-bearing text value and a "
            "byte-valued field that is not text"
        )
        assert (
