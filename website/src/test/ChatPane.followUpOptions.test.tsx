...
describe('ChatPane plan follow-ups dispatch (issue #5893)', () => {
  ...
  it('a failed dispatch releases its own class for retry — and only its own', async () => {
    // error → release is the ONLY HTTP-driven release (success consumes until
    // transcript ack). Per class: a rejecting Cancel must free retry-Cancel
    // while a still-pending Go stays latched.
    ;(api.planAction as ReturnType<typeof vi.fn>).mockImplementation(
      (_slot: string, action: string) => action === 'Cancel' ? Promise.reject(new Error('500')) : new Promise(() => {}),
    )
    ...
    await act(async () => { clickOption('Cancel') })  // rejects → releases the CANCEL set only
    await act(async () => { await Promise.resolve() })
    await act(async () => { clickOption('Cancel') })  // retry allowed after the failure
  ...
  it('a Go superseded by a Cancel on the same hook instance still runs its error release', async () => {
    ...
    await act(async () => { rejectGo(new Error('502')); await Promise.resolve() })
  ...
  it('a LATE failure from an old dispatch does not free a newer dispatch\'s latch', async () => {
    ...
    expect(api.planAction).toHaveBeenCalledTimes(2)
    await act(async () => { rejectFirstGo(new Error('late 504')); await Promise.resolve() })
