 ...
 *
 * - error   → released immediately (`onError`): a failed dispatch changed
 *   nothing server-side, so the user must be able to retry.
 ...
 *
 * Known trade-off: a dispatch that never settles keeps its slot latched
 * until a different plan row appears or a reload. On the Go map that is the
 * safe failure (a duplicate Go would be worse than a stuck one); on the
 * Cancel map it wedges the stop control itself for that slot — accepted as
 * the cost of double-row suppression, and bounded by a reload.
 */
...
/**
 * Test/reset escape hatch: drop both latches for a slot unconditionally.
 * Production release goes through the acknowledgement effect inside
 * `usePlanActionMutation` — do NOT call this from host code.
 */
export function releasePlanLatches(slot: string): void {
  goLatchBySlot.delete(slot)
  cancelLatchBySlot.delete(slot)
}
 ...
 * — see the latch lifecycle note above. A failed dispatch releases its class
 * immediately for retry; Cancel is never blocked by a pending Go. Hosts do
 * NOT add their own `isPending` pre-check — a render-scoped check would
...
export function usePlanActionMutation(slot: string | null, followUpSourceKey: string | null) {
  ...
  const mutation = useMutation({
    ...
    onError: (e, vars) => {
      // A failed dispatch changed nothing server-side — release for retry,
      // but ONLY if the latch still holds THIS dispatch's row: a late
      // failure (response lost, next footer already dispatched) must not
      // free a NEWER dispatch's latch and re-open a duplicate submit.
