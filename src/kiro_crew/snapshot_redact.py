...
def _redact_text_file(path: Path, report: RedactionReport, rel: str) -> bool:
    ...
    The NUL test alone does not cover every offset-dependent container, and which ones are
    left was MEASURED rather than reasoned about. Of ZIP, gzip, PNG, tar and a Flate-stream
    PDF, all five are already refused here -- each one is either not UTF-8 or carries NUL.
    The single realistic shape that reaches the rewrite is an uncompressed, pure-ASCII PDF,
    whose `xref` table holds byte offsets that a variable-length edit invalidates. So it is
    refused by its magic below. That is not a signature list waiting to grow: measurement
    says it is the one text-shaped container that gets here.

    Narrowed, not provably closed: a text-shaped format with internal offsets that is not
    PDF would still be rewritten and corrupted. The honest scope is that the reachable
    cases now refuse instead of silently shipping a broken off-host copy.
    """
    try:
...
def _refuse_update_triggers_that_destroy_rows(
    ...
    An FTS-maintenance trigger is exempt even when it deletes: deleting the stale index row
    is how such a trigger keeps the index in step, and the index is rebuilt after this pass
    regardless.

    The residual, stated rather than implied: this reads the trigger BODY for a DELETE, so a
    trigger that destroys a row by other means -- `INSERT OR REPLACE` over an existing key --
    is not caught. Bounding that needs the effects, not the text, which is a different piece
    of work from this refusal.
    """
    suspects: list[str] = []
    for name, tbl_name, sql in conn.execute(
        ...
        if tbl_name in fts_names or any(f in actions for f in fts_names):
            continue  # full-text index maintenance, rebuilt after this pass anyway
