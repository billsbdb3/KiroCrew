...
refuses if that pin fails. The approval picker offers Auto only when a
live goose session advertised that mode — picking it is an explicit
opt-in, and PreToolUse will not see those tools. pi
...
- **goose** asks per privileged tool only after Crew pins session mode
  `approve`. Its default `auto` auto-approves tools; Auto is an explicit
  picker opt-in on a live session that advertised it. **pi** asks via
