...
class _ChatSlot:
    ...
    def __init__(
        ...
        # Operator opt-in to a harness permission mode that skips
        # session/request_permission (goose ``auto``). Empty on every other
        # harness and on the default pin-to-approve path.
        self._harness_permission_mode: str = ""
...
class DashboardState:
    ...
    def serialize_slot(
        ...
        modes, current = self._live_slot_permission_auto(slot)
        if modes:
            payload["permission_modes"] = modes
            if current:
                payload["permission_mode"] = current
        return payload
    ...
    def _live_slot_permission_auto(self, slot: _ChatSlot) -> tuple[list[str], str]:
        """Advertised extra permission modes and the current one.

        Positive identity (H5): only goose session mode ``auto`` is a
        permission option. kiro uses set_mode for agent selection; an
        agent id ``auto`` must not appear here.
        """
        from kiro_crew.acp import goose as goose_backend
        from kiro_crew.acp.types import ACP_BACKEND_GOOSE

        client = self._live_slot_acp_client(slot)
        if client is None:
            return [], ""
        if getattr(client, "backend", None) == ACP_BACKEND_GOOSE:
            ...
            extra: list[str] = []
            if goose_backend.MODE_AUTO in available:
                extra.append(goose_backend.MODE_AUTO)
            current = ""
            live_mode = getattr(client, "_current_mode_id", "") or ""
            if live_mode == goose_backend.MODE_AUTO:
                current = goose_backend.MODE_AUTO
