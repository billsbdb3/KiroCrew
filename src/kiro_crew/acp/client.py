...
class AcpClient:
    ...
    def __init__(
        ...
        # Operator opt-in to goose session mode ``auto``. Empty means pin to
        # ``approve`` after session/new. Positive identity only (H5): kiro
        # uses set_mode for agent selection, so a kiro agent id ``auto``
        # must never land here.
        self._goose_permission_opt_in: str = ""
    ...
    async def set_goose_permission_mode(self, mode_id: str) -> None:
        ...
        runs. New sessions stay pinned to ``approve`` until the operator
        enables the single named ungated-tools opt-out and then opts in through
        the dashboard picker. A kiro agent id of ``auto`` must not take this
        path.
        """
        from kiro_crew.acp import goose as goose_backend
        from kiro_crew.acp.codex import Verdict
        ...
        if self.backend == ACP_BACKEND_GOOSE:
            if mode_id not in (goose_backend.MODE_APPROVE, goose_backend.MODE_AUTO):
            ...
            if mode_id == goose_backend.MODE_AUTO:
                try:
                    tool_gate.enforce_runtime_routing(
                        self.backend,
                        "goose session mode 'auto' auto-approves tool calls",
                        allow_ungated=self._allow_ungated_tools,
                        session_key=self._session_key or "",
                        verdict=Verdict.BYPASSED,
                    )
                    ...
                    raise AcpToolGateUnroutable(str(exc)) from exc
            self._goose_permission_opt_in = (
                goose_backend.MODE_AUTO if mode_id == goose_backend.MODE_AUTO else ""
            )
    ...
    async def _apply_goose_permission_mode(self) -> None:
        """Pin goose off its default ``auto`` onto ``approve``, unless opted in.

        goose 1.47+ starts every session in ``auto``, so a PERMISSION_REQUEST
        verdict that assumed it asks is wrong until this pin runs. An
        operator who picked Auto on this client keeps ``auto`` across
        re-init. ``smart_approve`` is also a bypass and is never pinned
        onto. Positive identity only (H5).
        """
        from kiro_crew.acp import goose as goose_backend
        from kiro_crew.acp.codex import Verdict

        if self._goose_permission_opt_in == goose_backend.MODE_AUTO:
            if not self._modes_advertised or goose_backend.MODE_AUTO in self._available_mode_ids:
                try:
                    await self.set_goose_permission_mode(goose_backend.MODE_AUTO)
                    return
                except AcpError as exc:
                    try:
                        tool_gate.enforce_runtime_routing(
                            self.backend,
                            "goose rejected session/set_mode to auto after "
                            "the operator opted in",
                            allow_ungated=self._allow_ungated_tools,
                            session_key=self._session_key or "",
                            verdict=Verdict.BYPASSED,
                            remedy=(
                                f"goose must accept session mode "
                                f"{goose_backend.MODE_AUTO!r}, or leave Auto."
                            ),
                        )
                    except tool_gate.ToolGateUnroutable as gate_exc:
                        raise AcpToolGateUnroutable(str(gate_exc)) from exc
        ...
        remedy = (
            ...
            "(Ask before every tool call). New sessions pin there unless the "
            "operator opts into auto."
        )
        if issue:
            try:
                tool_gate.enforce_runtime_routing(
