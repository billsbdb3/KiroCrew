...
- goose (`acp/goose.py`): `GOOSE_BIN`, then `mise which goose`, then the
  ...
  rejected — unless the named ungated-tools opt-out is on, or the
  operator has enabled the named ungated-tools opt-out and then opted this live
  session into Auto (the dashboard picker offers Auto only when both are true).
  Auto skips
  `session/request_permission`, so PreToolUse never runs. `smart_approve`
  is also a bypass and is not offered. File I/O stays in-process because
