...
`DISPATCHED` cannot pass admission again.
...
feed and the original request's live-generation watermark; a reconnect cannot
reinterpret an older coalesced response against a newer tombstone. A later
reconnect after settlement starts a fresh authoritative seed.
